...
%% Simulation setup
% Time
simulationTime_total = 45;           % in seconds *------* YOU CAN CHANGE THIS
...
% *------------------------------------------*
k = 1;
checkPointList = [ 1 2; 1.5 1; 3 -2; -1 -2; -0.7 -0.7];
checkPointCounter = 1;
tolerance = 0.03; 
...
for timeStep = 1:timeSteps_total
    ...
    [checkPointCounter,voltage] = fuzzyTaskOneController(booleanAtCheckPoint, checkPointCounter, error);
   ...
   % currentHeadingAngle = newHeadingAngle;
    ...
    checkPointListPlot = [ 2 1 ; 1 1.5 ; -2 3 ; -2 -1 ; -0.7 -0.7];
